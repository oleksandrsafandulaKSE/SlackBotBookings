...
- **`YAROOMS_EXCLUDED_SPACE_IDS`** (optional env var / Heroku Config Var): comma- or space-separated Yarooms space ids to keep out of every booking flow even though the API still returns them. Applied inside `_filter_target_spaces`, so excluded rooms never reach the cache, the pickers, the availability search or `check_space_live` (which reports them `"gone"`). Set to `1000020365,1000020366` in production — the decommissioned `[0.1]` / `[0.2]` Shelter Skype Rooms, which are still live in the Yarooms API.
